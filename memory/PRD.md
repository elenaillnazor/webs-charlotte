...
Actualización 4 (2026-09-25): nueva sección "Encargos" (tarta-helado para celebraciones, foto real de tarta con vela, CTA llamar + Instagram) incluida en nav y footer. Salado: mención explícita "todos nuestros salados son en croissant" (blurb + nota) con foto real del bocata de croissant; Tostadas: foto real de la tosta de salmón; Bocadillos: foto de bocadillo. Galería ajustada (sundae, tostada de aguacate, galletas).
Reseñas en vivo: backend /api/google-reviews implementado (Places API New, clave y place_id en backend/.env; devuelve configured:false y el frontend muestra el 4,7 estático hasta configurar la key de Google Cloud del cliente). Pendiente: clave GOOGLE_PLACES_API_KEY + GOOGLE_PLACE_ID del cliente; foto real de Bebidas.
Decisión cliente (2026-09-25): NO se usará la integración de Google Places — el 4,7 se muestra estático (funciona y queda apagado, sin coste).
Actualización 3 (2026-09-25): sustituidos los testimonios simulados por las 4 reseñas reales de Google (David Barroeta, Daniel, Sylvana, Gabriel Vicente P., todas 5★, en rejilla 2x2). Eliminada toda mención a "46 reseñas": solo se comunica el 4,7★ y "uno de los locales mejor valorados de Valdemoro" (héroe, barra de confianza, resumen de opiniones). Horario real integrado: Lun–Vie 9:00–22:00, Sáb–Dom 10:00–22:00 (contacto + footer); indicador "Abierto ahora" respeta la hora de apertura por día. Nuevas fotos reales: café con croissant → panel Café; cono servido frente a la vitrina → panel Helados; galletas con café → galería.
